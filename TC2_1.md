...
### Exemplo: seja o conjunto $K$ novamente tal que $K = \{x \in \mathbb{N} \mid \Phi(x,x)\downarrow \}$. Mostre que $K$ não é recursivo.
...
### Exemplo: Prove que $TOT = \{z \in \mathbb{N} \mid (\forall x)\Phi(x,z)\downarrow\} não é r.e..$
...
### Exemplo: Uso do $K$ e $\overline{K}$

Agora eu posso pegar o exemplo de $K$ e $\overline{K}$ para mostrar que um conjunto não é recursivo ou não é r.e..

**Para mostrar que não é recursivo:** $K \leq_m B $ implica em $x \in K \iff f(x) \in B$

**Para mostrar que não é r.e.:** $\overline{K} \leq_m B $ implica em $x \in \overline{K} \iff f(x) \in B$

Esse $f$ é uma função desconhecida. Para descobrir, é preciso do teorema 3.13 seguinte:
...
É suficiente, pra esse caso, provar que $\overline{K} \leq_m EMPTY$.
...
```
     Z ← Phi(X_2,X_2)
```
A engenhosidade aqui é usar a variável $X_2$ e deixar a variável $X_1$ totalmente livre.
...
Dado este programa, temos a seguinte relação:

- $\psi_{\mathcal{P}}(x,z)\uparrow$ para todo $x$ se e somente se $\Phi(z,z)\uparrow$.

Veja que $\Phi(z,z)\uparrow$ está associado ao $\overline{K}$, e o $\psi_{\mathcal{P}}(x,z)\uparrow$ está associado ao $EMPTY$.
...
- $\Phi(x_1,x_2)\uparrow \iff \forall x_1 \Phi(x_1,x_2,p)\uparrow$
- $\Phi(x_1,x_2)\downarrow \iff \forall x_1 \Phi(x_1,x_2,p)\downarrow$

**PASSO 3:**


$$x_2 \in \overline{K} \iff \Phi(x_2, x_2) \uparrow \iff \forall x_1 \Phi(x_1,x_2,p)\uparrow \text{( Teorema do Parâmetro)} \iff \forall x_1 \Phi(x_1, S^1_1(x_2,p))\uparrow \iff W_{S^1_1(x_2, p)} = \emptyset \iff S^1_1(x_2, p) \in EMPTY \iff f(x_2) \in EMPTY$$

Perceba que $x_2 \in \overline{K} \iff f(x_2) \in EMPTY$ Portanto, $EMPTY$ não é r.e..

**OBSERVAÇÃO IMPORTANTÍSSIMA**

Quando resolver qualquer questão de redução, utilize estritamente o estilo de método de prova do teorema 3.14, com passo a passo da mesma forma. Ou seja, use e abuse do "se e somente se". Faça do passo 1 o programa, o passo 2 e 3 é o resto da prova.